...
- `x-expose` — restricts which surfaces include this mount's entry under `?surface=<name>`: a string or array of surface names (`"mcp"`, `"ui"`, `"cli"`). **Absent = every surface.** Filtering applies **only to `GET /.well-known/rs2/agent-surface`** (the `/services` catalogue and `OPTIONS` are *not* filtered by `?surface=`), and only to mounts that appear there at all — data mounts (`entity`), pipeline specs (`action`), and query specs (`query`); **file mounts never appear on the agent surface**, so `x-expose` on a `file` mount is inert. Use it e.g. to limit an internal action to `["mcp","cli"]` (off the `ui` surface), or a query to `["mcp"]`.
