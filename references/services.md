...
Beyond the store contract: a single `Range:` on GET → 206 with `Accept-Ranges: bytes` and `Content-Range: bytes <first>-<last>/<total>` (all three forms — `bytes=0-1023`, `bytes=1024-`, and the suffix `bytes=-1024` for the last N bytes; the end is inclusive and clamps rather than failing). A start at or past the end is **416** carrying `Content-Range: bytes */<total>`; anything the server won't satisfy (another unit, a multi-range set, an invalid `bytes=500-400`) is **ignored** — a plain 200 with no `Content-Range`, so check for 206 rather than assuming. `If-Range` gates the slice on a strong validator (send the ETag you started with: 206 to resume, 200 with the whole new resource if it changed); `If-None-Match` is evaluated first, so a fresh copy is a 304 even with a `Range`. Also `Last-Modified`; `HEAD` for metadata headers (`Content-Length`, `Content-Type`, `ETag`, `Accept-Ranges` — the same validator GET serves, so a client can check a file's version without downloading it). A GET/HEAD whose path names a directory **without** the trailing slash answers **301** to the slash form (query preserved, `DirectorySlash` style — relative URLs in a served default doc need the slash form); what the slash form yields (default doc, listing, or 404) follows the rules below. Writes stream end-to-end (no materialization), atomically (temp + rename). Keyless POST names the file `<uuid><ext>` with the extension inferred from the request `Content-Type`. Media types come from the extension map, never sniffed; unknown → `application/octet-stream`. The map is comprehensive (~1200 extensions, the IANA + Apache + nginx aggregate), so video, audio, fonts and office documents serve with the type a browser needs; `.ts` and `.tsx` are TypeScript source, not MPEG transport streams. Storage is host-scoped per tenant.
...
- `listings: false` — suppresses dir+json listings (404), so a public site isn't browsable. Default `true`. **Concealment, not access control**: files stay readable by name under `access.read`, so don't use it to protect anything — use `access`. Tenant **operators** are exempt (they can flip the flag anyway, and hiding a mount's inventory from its owner only breaks the CLI/agent view): an operator sending an explicit `Accept: application/vnd.rs2.dir+json` gets the listing, returned `Cache-Control: no-store` + `Vary: Accept, Authorization, Cookie` so a shared cache can't serve it on to the public. Operators browsing normally still get the default doc. The mount's `access.read` is checked first as always — the exemption lifts concealment, never authorization.
