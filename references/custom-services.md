...
- `msg`: `{ method, url (path+query), headers (object), body, mediaType }` — the body is converted by media type: JSON arrives parsed, text (`text/*`, `application/xml`, `application/xhtml+xml`, `application/javascript`, `application/typescript`) as a string, and anything else as a **standard base64 string** (decode with `atob`) so binary crosses intact. A body typed `application/json` that does not parse is a 400.
...
- `ctx.request(capability, { method?, url, headers?, body?, mediaType? })` — the only way out of the sandbox; synchronous from JS (no await needed, though awaiting works). Returns `{status, headers, body, mediaType}`, the body converted the same way as `msg.body` (parsed for JSON, string for text, base64 for binary). An ungranted capability throws an Error with `e.code === "capability_denied"`; uncaught, it fails the invocation with that same structured error.
...
- **`{"type": "images", "source": "source", "cache": "cache"}`** — host-side **image transforms by reference** (Cloudflare host; the Rust host accepts the grant so the config is portable but answers 501 `provider_unavailable` at use). `source` and `cache` name sibling grants: the host reads the original through `source` (a `prefix` grant — the caller's identity applies) and writes derivatives through `cache` (a `store` grant), so no image bytes cross into the sandbox. The guest calls `ctx.request("images", { url: "/info?source=/photo.jpg" })` → `{width, height, mediaType, bytes}` (415 when not decodable), or `{ url: "/transform?source=/photo.jpg&w=640&h=480&fit=cover&g=0.5,0&rect=x,y,w,h&f=jpeg&q=78&maxSourcePixels=16000000&store=/d/ab/key.jpg" }` → the derivative written at `cache:/d/ab/key.jpg` and `{mediaType, bytes, stored}` back; without `store` the transform bytes are the response (use it as an `x-rs2-body-ref` target). `fit` is `scale-down` | `contain` | `cover` | `fill`; `g` is `x,y` fractions of the leftover space; `f` is `jpeg` | `png` | `webp` | `avif`; 413 above `maxSourcePixels`.
...
The rs2-runtime repo ships one image service as two deployables with the **same mount contract** — pick by host: `guest-services/image` is a Wasm component (codecs inside; the Rust host), `guest-services/image-js/image.js` is a JS bundle that transforms through the host's `images` grant (the Cloudflare host; add `"images": {"type": "images", "source": "source", "cache": "cache"}` to the grants below — the Rust host accepts that grant too, so one tenant file mounts on both). Either gives any file mount query-string resize/crop for responsive design: mount `code:image@<version>` with a `source` prefix grant (originals; caller authz preserved) and a `cache` store grant (derivatives). `GET /img/photo.jpg?w=640` — params `w`/`h` (px), `dpr` (1–3), `fit` (`scale-down` default | `contain` | `cover` | `fill`), `g` (cover gravity: compass or `x,y` fractions), `rect=x,y,w,h` (pre-crop), `f` (`auto`|`jpeg`|`png`|`webp`-lossless), `q` (1–100); `?$info` returns `{width,height,mediaType,bytes}`; no params = passthrough; `DELETE /<mount>/.cache?confirm=` purges (guarded by the mount's `delete` access). Unknown params are 400. Derivatives carry a strong ETag keyed on the source ETag + canonical params (304s work; edits auto-invalidate); cache hits and passthrough are served via `x-rs2-body-ref` (no bytes through the sandbox); config `widths: [320, 640, …]` snaps width-only requests up a ladder to bound cache cardinality, `maxSourcePixels` (default 16 MP) guards decode. Build `cargo build --target wasm32-wasip2 --release` (Wasm) or deploy `image.js` as-is (JS; `rs2 deploy guest-services/image-js/image.js --name image`), add mount `caching` config for Cache-Control. Full config shape: each bundle's README. Output codecs differ by host (the Wasm component's WebP is lossless; the Cloudflare backend also encodes `avif` and lossy WebP), so compare derivatives by dimensions and type, never bytes. Local `wrangler dev` emulates only width/height/format; cover/gravity/rect need a deployed Worker.
